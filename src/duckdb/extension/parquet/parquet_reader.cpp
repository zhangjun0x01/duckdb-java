...
bool ParquetReader::CheckProjectionMatch(const vector<string> &project_columns, string full_name) {
	if(project_columns.empty()) {
		return true;
	}
	// For list<struct> type, the parquet metadata generated by iceberg is similar to 'content.list.element.level',
	// and the 'list.element' is fixed, to avoid user write it and provide convenience for user, we remove 'list.element'
	const static string list_element = ".list.element";
	full_name = full_name.find(list_element) != std::string::npos ? full_name.replace(full_name.find(list_element), list_element.length(), "") : full_name;

	//Compared with iceberg, the parquet metadata generated by c++ sdk lacks 'element', is similar to ’content.list..level‘ , we are compatible with this case.
	const static string list_element_end = ".list.";
	full_name = full_name.find(list_element_end) != std::string::npos ? full_name.replace(full_name.find(list_element_end), list_element_end.length(), "") : full_name;

	//针对list<struct>类型，类似content.list的前缀也要匹配.
	const static string list_element_nested = ".list";
	full_name = full_name.find(list_element_nested) != std::string::npos ? full_name.replace(full_name.find(list_element_nested), list_element_nested.length(), "") : full_name;

	for (const auto& column : project_columns) {
		// equal or start with or regex match
		if(column == full_name || column.rfind(full_name, 0) == 0 || std::regex_match(full_name, std::regex(column))){
			return true;
		}
	}
	return false;
...
unique_ptr<ColumnReader> ParquetReader::CreateReaderRecursive(ClientContext &context, idx_t depth, idx_t max_define,
                                                              ...
                                                              idx_t &next_file_idx, const string& parent_name) {
	...
	if (s_ele.__isset.num_children && s_ele.num_children > 0) { // inner node
		...
		while (c_idx < (idx_t)s_ele.num_children) {
			...
			// ECS MARK:
			// the full_name is similar to 'content','content.list','content.list.element','content.list.element.level','content.list..level',
			// compare the full_name with parquet_options.projection_columns.
			auto full_name = parent_name.empty() ?  child_ele.name : parent_name + "." + child_ele.name;
			auto child_reader =
				CreateReaderRecursive(context, depth + 1, max_define, max_repeat, next_schema_idx, next_file_idx, full_name);

			//If the column is not match, we do not add the child_reader to its parent's reader
			//so that we can skip read the relevant column when read the data from parquet file.
			bool match = CheckProjectionMatch(parquet_options.projection_columns, full_name);
			if(match) {
		...
		// ECS MARK:
		// When some columns do not match the parquet_options.projection_columns, we do add it to child_types.
...
unique_ptr<ColumnReader> ParquetReader::CreateReader(ClientContext &context) {
	...
	// ECS MARK:
	// When some columns do not match the parquet_options.projection_columns, we will skip the column reader.
